...
class CafeForm(FlaskForm):
    ...
    ratingcoffee = SelectField('Coffee Rating', choices=coffee)
    ratingwifi = SelectField('WIFI Strength Rating', choices=wifi)
    ...
    if form.validate_on_submit():
        with open('cafe-data.csv', 'a', encoding='utf-8') as data:
            data.write(f"\n{form.cafe.data},"
                           ...
                           f"{form.open.data},"
                           f"{form.close.data},"
                           f"{form.ratingcoffee.data},"
                           f"{form.ratingwifi.data},"
                           f"{form.power.data}")
